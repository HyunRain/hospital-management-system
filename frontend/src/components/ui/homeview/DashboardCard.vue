...
<template>
  <article class="flex flex-col p-5 shadow-md rounded-lg bg-white dark:bg-[#0a0a0a] dark:border dark:border-neutral-900">
    <header class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <img class="h-6 w-6" :src="imgSrc" alt="Card Icon" />
        <h2 class="font-medium text-[16px]">{{ props.title }}</h2>
      </div>
      <button type="button" class="cursor-pointer">
        <img class="h-6 w-6" :src="`/assets/icons/${toggleStore.darkModeState}/options.svg`" alt="More options" />
      </button>
    </header>

    <section class="flex items-center mt-6 justify-between">
      <p class="text-2xl font-bold" aria-label="Amount">{{ props.amount }}</p>

      <div class="flex items-center gap-1 rounded-lg p-1" aria-label="Trend indicator">
        ...
        <output :class="textColor">{{ props.trend >= 0 ? '+' : '' }}{{ props.trend }}%</output>
      </div>
    </section>

    <footer class="mt-3 text-[#898989]">
      <small>32 more than last week</small>
    </footer>
  </article>
</template>
