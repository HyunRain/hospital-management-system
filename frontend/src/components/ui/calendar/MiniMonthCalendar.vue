...
function handleClick(day: { value: number, type: string; }) {
  ...
  emit('dateSelected', {
    localDate: currentYear.value + '-' + String(currentMonth.value + 1).padStart(2, '0') + '-' + String(day.value).padStart(2, '0'),
  });
  emit('toggle');
}
  ...
  <div class="flex flex-col">
    ...
    <div class="grid grid-cols-7 grid-rows-[auto_1fr] h-full">
      ...
      <div class="grid grid-cols-7 col-span-7 overflow-hidden h-full">
        <div v-for="(day, index) in totalDaysForCurrentMonth" @click.stop="handleClick(day)"
