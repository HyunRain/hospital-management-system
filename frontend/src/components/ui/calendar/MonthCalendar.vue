...
const { getAppointmentsForDay } = useCalendar();
      ...
      <section v-for="(day, index) in props.totalDaysForCurrentMonth" @click="handleClick(day)"
        ...
        <section class="flex-1 flex-col items-start justify-center relative" :style="{ minHeight: gridRowHeight }">
          <MonthAppointmentCard
            v-for="(appointment, indexApp) in getAppointmentsForDay(day, props.currentMonth, props.currentYear).appointments.slice(0, 3)"
            :key="indexApp" :appointment="appointment"></MonthAppointmentCard>
          <p @click.stop="toggleStore.toggleFullMonthAppointmentCards(index)"
            v-if="getAppointmentsForDay(day, props.currentMonth, props.currentYear).length > 3"
          ...
          <div
            class="absolute top-full bg-neutral-200 dark:bg-[#101010] z-50 border dark:border-neutral-800 border-neutral-200 px-2 py-2 rounded-lg select-none min-w-[250px] left-1/2 -translate-x-1/2"
            ...
            <MonthAppointmentCard v-for="(appointment, indexApp) in getAppointmentsForDay(day, props.currentMonth, props.currentYear).appointments"
              :key="indexApp" :appointment="appointment"></MonthAppointmentCard>
          </div>
