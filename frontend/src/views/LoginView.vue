...
<template>
  <div class="flex bg-gray-50 dark:bg-zinc-900 w-full justify-center items-center">
    <div class="flex max-w-[1000px] justify-center w-full md:shadow-lg rounded-xl h-[65%]">
      ...
      <div class="px-5 md:p-0 items-center justify-center bg-white dark:bg-[#1c1c20] flex flex-col md:w-1/2 rounded-r-xl">
        ...
        <form class="flex flex-col items-center justify-center gap-6 w-full md:w-2/4" @submit.prevent="handleLogin()">
          <input
            ...
            type="email" name="email" placeholder="Email" v-model="loginInput.email" required />
          <input
            class="bg-white dark:bg-[#eeeeee] text-[#212121] placeholder-[#4f4f4f] w-full px-[10px] py-[7px] rounded-xl border border-gray-300 dark:border-0 dark:focus:ring-0 focus:ring-1 focus:outline-none focus:ring-red-300 transition"
            type="password" name="password" placeholder="Password" v-model="loginInput.password" required />
          <button
        ...
        <p class="mt-5 cursor-pointer underline hover:text-[#a7a7a7]">Explore as a Demo Admin</p>
      </div>
    </div>
  </div>
</template>
